    ...
    group(
        ...
        () => {
              test('user-not-found エラーメッセージ', () {
                ...
                final message =
                    ErrorHandler.getFirebaseAuthErrorMessage(exception);
    ...
    group(
        ...
        () => {
              test('permission-denied エラーメッセージ', () {
                ...
                final message =
                    ErrorHandler.getFirestoreErrorMessage(exception);
    ...
    group(
        ...
        () => {
              test('invalid_format エラー', () {
                final message =
                    ErrorHandler.getQRErrorMessage('invalid_format');
              ...
              test('user_not_found エラー', () {
                final message =
                    ErrorHandler.getQRErrorMessage('user_not_found');
                expect(message, 'このユーザーは存在しません');
              }),
              test('already_friend エラー', () {
                final message =
                    ErrorHandler.getQRErrorMessage('already_friend');
                expect(message, '既にフレンドです');
              }),
            });

    group(
        'Anime List Error Messages',
        () => {
              test('fetch_failed エラー', () {
                final message =
                    ErrorHandler.getAnimeListErrorMessage('fetch_failed');
                expect(message, 'アニメリストの取得に失敗しました');
              }),
              test('save_failed エラー', () {
                final message =
                    ErrorHandler.getAnimeListErrorMessage('save_failed');
                expect(message, 'アニメの保存に失敗しました');
              }),
              test('empty_selection エラー', () {
                final message =
                    ErrorHandler.getAnimeListErrorMessage('empty_selection');
    ...
    group(
        ...
        () => {
              test('update_failed エラー', () {
                final message =
                    ErrorHandler.getProfileErrorMessage('update_failed');
                expect(message, 'プロフィールの更新に失敗しました');
              }),
              test('invalid_username エラー', () {
                final message =
                    ErrorHandler.getProfileErrorMessage('invalid_username');
                expect(message, 'ユーザー名が無効です');
              }),
              test('username_too_long エラー', () {
                final message =
                    ErrorHandler.getProfileErrorMessage('username_too_long');
